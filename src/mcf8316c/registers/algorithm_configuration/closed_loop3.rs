...
pub struct ClosedLoop3 {
    ...
    pub curr_loop_kp: KVal<CurrentKp>,
    /// 10-bit value for current Iq and Id loop Ki. 0 = Auto
    pub curr_loop_ki: KVal<CurrentKi>,
    /// 3 MSB for speed loop Kp.
    pub spd_loop_kp: SpeedLoopKpHigh3,
}

impl Register for ClosedLoop3 {
    const ADDRESS: u16 = CLOSED_LOOP3; // Example address, replace with actual address
}

impl From<ClosedLoop3> for u32 {
    fn from(config: ClosedLoop3) -> Self {
        let mut value = 0;
        value |= (config.motor_bemf_const as u32) << 23;
        value |= (Into::<u16>::into(config.curr_loop_kp) as u32) << 13; // 10 bits
        value |= (Into::<u16>::into(config.curr_loop_ki) as u32) << 3; // 10 bits
        value |= config.spd_loop_kp.0 as u32; // 3 bits
        value
    }
}

impl From<u32> for ClosedLoop3 {
    fn from(value: u32) -> Self {
        ClosedLoop3 {
            motor_bemf_const: MotorBemf::from(((value >> 23) & 0xFF) as u8),
            curr_loop_kp: KVal::try_from(((value >> 13) & 0x3FF) as u16).unwrap(),
            curr_loop_ki: KVal::try_from(((value >> 3) & 0x3FF) as u16).unwrap(),
            spd_loop_kp: SpeedLoopKpHigh3((value & 0x7) as u8),
        }
    }
}
